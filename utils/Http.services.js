...
import { getToken, getObject, setObject, getVToken } from "./useful";
import cookie from "cookie";

const requestSecurity = new Security();

class HttpService {
  async syncRequest(action, params, context) {
    ...
    if (api) {
      ...
      console.log(this.serviceMethod(api, hashedData, context));
      let res = await fetch(
        siteConfig.domain + "apiv1",
        this.serviceMethod(api, hashedData, context)
      );
      ...
      this.checkParams(api, params, (newParams, paramsError) => {
        ...
        if (!paramsError) {
          ...
          this.handleFiles(api, newParams, () => {
            requestSecurity.hashData(
              newParams,
              api,
              (hashedData, hashError) => {
                if (!hashError) {
                  // console.log(hashedData)
                  // console.log(this.serviceMethod(api, hashedData))
                  fetch(
                    siteConfig.domain + "apiv1",
                    this.serviceMethod(api, hashedData)
                  )
                    .then((res) =>
                      this.handleResponseError(res, (result, err, status) => {
          ...
          new Promise((resolve, reject) => {
            this.fileUpload(value, newParams["applet"], key, (fileInfo) => {
              ...
              if (Array.isArray(fileInfo)) {
                newParams[key] = Array.isArray(newParams[key])
                  ? newParams[key]
                  : [];
  ...
  serviceMethod(api, data, context) {
    let request;
    ...
    }; //, 'Authorization': 'bearer ' + Auth.getToken(), ...custom_header });
    ...
    //     let token = getVToken()
    //     if (token) {
    //         headers.vAuthorization = "bearer " + token
    //     } else {
    //         /// TO DO
    //     }
    // }
