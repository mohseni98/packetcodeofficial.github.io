...
function changeUrlWithLng(lng) {
    ...
    if (lng === siteConfig.defaultLng && siteConfig.dontShowUrlForDefaultLng) {
        ...
        if (lng !== pathname[0]) {
            ...
            window.location = "/" + pathname.join('/') + (search ?? '')
...
function imageAddress(image, type, prefered) {
    // console.log(image)
    let imageInfo = {
        profile: { address: 'uploads/', defaultImg: '/assets/default/user.svg' },
        product: { address: 'uploads/', defaultImg: '/default/noimageavailable.jpg' },
        resource: { address: 'uploads/', defaultImg: '/default/noimageavailable.png' },
        estate: { address: 'uploads/', defaultImg: '/default/def.jpg' },
        tickeProfile: { address: 'uploads/', defaultImg: '/default/man-1.svg' },
...
function getToken() {
    ...
    // }
    // }
    // token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjYwZDFiZjdhMTRhN2IyZjZhMWU1YTc0ZCIsImlhdCI6MTY0NTYyNDY2MH0.gZw4bSgVolp0KZt9xoeoSzXCLvrIZRK6bP18wNDZNhc"
...
export { msToHMS, getVToken, translate, getBoundingClientRect, convertToSlug, detectBrowser, togglePin, getOneThing, changeBodyDirection, changeUrlWithLng, currentLng, checkTextTranslation, checkTranslation, pathMaker, imageAddress, getToken, getUserInfo, phoneStandardView, getObject, setObject, capitalLetter, chooseWallpaper, priceStandardView, converNumberToPersian, fetchWallpaper, filterLabelCreator }